...
def handler_admin(message):
    ...
    cursor = conn.cursor()
    row = cursor.execute(f'SELECT * FROM settings WHERE id = 1').fetchone()
    if row is not None and (str(chat_id) in row[3] or str(chat_id) in row[4]):
    ...
    if message.chat.type == 'private':
        ...
        if len(row) == 0:
        	...
        	try:
        		...
        		pass
        bot.send_message(chat_id, """<b>🪐 Добро пожаловать</b> в Гарант Night 🪐

Гарант Night – это программа, которая обеспечивает безопасность при совершении сделок в мессенджере Telegram. Он предназначен для защиты покупателей и продавцов от мошенничества и обмана.

Гарант бот работает следующим образом: при совершении сделки между покупателем и продавцом, деньги переводятся на временный счет бота. После этого продавец отправляет товар покупателю, а покупатель проверяет его и подтверждает получение. Если товар соответствует описанию и качеству, то деньги переводятся продавцу. Если же товар не соответствует описанию или не был отправлен вовремя, то деньги возвращаются покупателю.⠀
""", parse_mode="HTML", reply_markup=menu.main_menu)
        conn.close()
...
def handler_start(message):
    ...
    if message.chat.type == 'private':
        ...
        bot.send_message(chat_id, """<b>Помощьb</b> Для начала я бы хотел пригласить тебя в свою империю.


Бот создан для проведения безопасных сделок, если вас заскамили то для начала нужно полностью прочитать условия.
...
Сервис взимает комиссию в 4%
[И комиссия от платежных систем]

   🎖Статистика:
    ├ Сервису более 3 - месяца
    ├ Пользователей: 712
    ├ Проведено сделок: 17
    └ На сумму: 1844₽
⠀
	...
	if message.chat.type == 'private':
		...
		if message.chat.type == 'private':
			if row is not None and (str(chat_id) in row[3] or str(chat_id) in row[4]):
				if msg == 'Статистика':
					...
					try:
						...
						bot.send_message(chat_id, '😵 Ошибка 😵\n😕 Нет такого пользователя в базе данных или он не зарегистрирован')
			...
			if msg == '💬 Помощь':
				...
				if rows != []:
				    for row in rows:
				        ...
				        sales.add(
				            ...
				            types.InlineKeyboardButton(text='🧑‍✈️ Открыть спор', callback_data='dispute '+str(row[0])),
				        )
				...
				bot.send_message(chat_id, "🔎Введите никнейм в формате @username или id пользователя")
			elif msg[:1] == "@":
				name = msg[1:]
				...
				try:
				    ...
				    garant_user.add(
				    	types.InlineKeyboardButton(text='Предложить сделку', callback_data='oplata '+row[0]),
				...
				text =  '<b>🎩Накинь монетку в сокровищницу Knight, каждый донат отправится в поддержку проекта</b>\n\n'
	...
	if chat_id in click:
		...
		try:
			...
			if a[0] == 'how':
				...
				send = bot.send_message(chat_id, "Введите сумму пополнения(от 110 руб)")
				bot.clear_step_handler_by_chat_id(chat_id)
				...
				text += '\n⚠️ОСТОРОЖНО!</b>\nЖертвуя в данный Проект, ты автоматически становишься частью <b>Нашей Империи.</b>'
				bot.edit_message_text(
					chat_id=chat_id,
					message_id=message_id,
					text=text
				...
				try:
					...
					if res == 'PAID':
						...
						text = f"<b>🎉Поступил донат!\n\n🔑 Дорогой пользователь: @{call.message.chat.username} становится частью нашей Империи <b>Knight</b>\n\n💰 Сумма: </b>{a[2]} руб.\n<b>🏅 Он занимает №{k} место в топе Донатеров</b>\n\n🎩Рыцарь лично снимает шляпу перед тобой."
						text2 = "<b>🎩Добро пожаловать в Рыцарскую Империю\n\n▪️*цзиньк*</b>, Монета упала в хранилище с золотом\n\n▪️ Благодарю тебя за донат, и снимаю перед тобой рыцарский шлем\n\n🖤 Эти средства пойдут на <b>Развитие Проекта!</b>"
	...
	try:
		...
		if s >= 1:
			...
			if check == 1:
				...
				text2 = "<b>🎩Добро пожаловать в Рыцарскую Империю\n\n▪️*цзиньк*</b>, Монета упала в хранилище с золотом\n\n▪️ Благодарю тебя за донат, и снимаю перед тобой свой рыцарский шлем.\n\n🖤 Эти средства пойдут на <b>Развитие Проекта!</b>"
				bot.send_message(chat_id, text2)
				try:
					bot.send_message(rowss[5], text, reply_markup=menu.donat_keyboard)
				except:
...
def input_qiwi(message):
	...
	try:
		...
		if i >= 10 :
			...
			bot.send_message(chat_id, "Минимальная сумма пополнения - 110р", reply_markup=menu.main_menu)
	except:
		bot.send_message(chat_id, "Ошибка", reply_markup=menu.main_menu)
